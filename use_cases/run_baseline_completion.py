...
import click as ck
import os
from src.baseline_completion import BaselineCompletion
...
@ck.option('--graph_type', '-g', required=True, type=ck.Choice(["owl2vec"]))
...
def main(use_case, graph_type, emb_dim, batch_size, lr, num_negs,
         margin, test_batch_size, epochs, device, seed, only_train,
         only_test, not_sweep, description):


    wandb_logger = wandb.init(project="cate2", name=description, group=f"{graph_type}_completion_{use_case}")
    ...
    model = BaselineCompletion(use_case,
                               graph_type,
                          root,
                          emb_dim,
                          batch_size,
