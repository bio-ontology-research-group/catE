...
class UnsatDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, data, c_dict):
        super().__init__()
        self.cfg = cfg
        self.c_dict = c_dict
        self.data = torch.tensor(self.parse_data(data))
    ...
    def parse_data(self, data):
        ...
        for axiom in data:
            ...
            tensor.append([self.c_dict[left]])
...
class FALCON(torch.nn.Module):
    ...
    def forward_unsat(self, class_emb, anon_e_emb):
         c_emb = self.c_embedding(class_emb).squeeze()
         c_emb = self._get_c_fs(c_emb, anon_e_emb)
         bot = self.nothing
         return self._logical_and(c_emb, self._logical_not(bot))
...
def extract_nodes_test(cfg, filename, moreAxioms=[]):
    ...
    with open(cfg.data_root + filename) as f:
        for line in f:
            line = line.strip('\n')
            concept, bot = line.split(',')
            line = f'ObjectIntersectionOf(<{concept}> ObjectComplementOf({bot}))'
            flag = 0
            for outlier in outliers:
                if outlier in line:
                    flag = 1
...
def unsat_get_logits(model, loader, c_dict, anon_e_emb, device):
    model.eval()
    Y = []
    logits = []
    ...
    with torch.no_grad():
        for X, y in loader:
            X = X.to(device)
            logit = model.forward_unsat(X, anon_e_emb).unsqueeze(0)
            logits.append(logit)
            Y.append(y)
...
def unsat_compute_metrics(logits, ys, c_dict):
...
def unsat_evaluate(model, loader, c_dict, anon_e_emb, device):
    model.eval()
    mr, mrr, mh1, mh3, mh10, mh50, mh100 = 0, 0, 0, 0, 0, 0, 0
    ranks = dict()
    with torch.no_grad():
        for X, y in loader:
            X = X.to(device)
            logits = model.forward_unsat(X, anon_e_emb)
            r, rr, h1, h3, h10, h50, h100 = get_ranks(logits, y[0])
            mr += r
            mrr += rr
            mh1 += h1
            mh3 += h3
...
if __name__ == '__main__':
    ...
    unsat_dataset_test = UnsatDataset(cfg, tbox_test_pos, c_dict)
    unsat_dataloader_test = torch.utils.data.DataLoader(dataset=unsat_dataset_test, 
                                                        batch_size=1,
                                                        # num_workers=4,
                                                        shuffle=False,
                                                        drop_last=False)
    ...
    for i in range(cfg.n_models):
        ...
        for step in range(cfg.max_steps):
            ...
            model.train()
            anon_e_emb_1 = model.e_embedding.weight.detach()[:cfg.anon_e//2].to(device) + torch.normal(0, 0.1, size=(cfg.anon_e//2, cfg.emb_dim)).to(device)
            ...
            if (step + 1) % cfg.valid_interval == 0:
                ...
                preds = []
                with torch.no_grad():
                    mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = unsat_evaluate(model, unsat_dataloader_test, c_dict, anon_e_emb, device)
        ...
        logits_this_model, ys = unsat_get_logits(model, unsat_dataloader_test, c_dict, anon_e_emb, device)
        logits.append(logits_this_model.unsqueeze(0))

        mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = unsat_compute_metrics(logits_this_model, ys, c_dict)
        ...
        mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = unsat_compute_metrics(agg_preds_so_far, ys, c_dict)
