...
import pandas as pd
from mowl.nn import ELEmModule, ELBoxModule, BoxELModule, BoxSquaredELModule
...
class ELModel(EmbeddingELModel) :
    ...
    def __init__(self, use_case, model_name, root, num_models,
        ...
        if "go" in use_case:
            train_path = os.path.join(self.root, "go.train.owl")
        elif "foodon" in use_case:
            train_path = os.path.join(self.root, "foodon-merged.train.owl")
            
        self.validation_file = os.path.join(self.root, "valid_cleaned.csv")
        self.test_file = os.path.join(self.root, "test_cleaned.csv")
        self.ancestors_path = os.path.join(self.root, "inferred_ancestors.txt")

        
        raw_dataset = PathDataset(train_path)
        # raw_class_to_id = {c: i for i, c in enumerate(self.raw_dataset.classes.as_str)}

        
        # train_ont = self.preprocess_ontology(raw_dataset.ontology)
                
        dataset = Dataset(raw_dataset.ontology)
    ...
    def create_inferred_subsumption_dataloader(self, tuples_path, batch_size):
        ...
        for h, t in zip(tuples["head"], tuples["tail"]):
            if h in class_to_id and t in class_to_id:
                heads.append(class_to_id[h])
                tails.append(class_to_id[t])
        print(f"Number of inferred subsumptions: {len(heads)}")
    ...
    def train(self, logger):
        ...
        valid_dl = self.create_subsumption_dataloader(self.validation_file, self.test_batch_size)
    ...
    def get_filtering_labels(self):
        ...
        num_testing_tails = len(self.dataset.classes)
        ...
        with th.no_grad():
            for head_idxs, tail_idxs in tqdm(testing_dataloader, desc="Getting labels"):
                ...
                for i, head_id in enumerate(head_idxs):
                    tail_id = tail_idxs[i]
                    filtering_labels[head_id, tail_id] = 10000
    ...
    def compute_ranking_metrics(self, filtering_labels=None, mode="test", model=None):
        ...
        if "test" in mode:
            print(f"Loading best model from {self.model_filepath}")
            self.load_best_model()
            
        test_file = self.test_file if mode == "test" else self.validation_file
        ...
        eval_dl = self.create_subsumption_dataloader(test_file, self.test_batch_size)
    ...
    def predict(self, heads, tails, mode, model=None):
        ...
        heads = heads.repeat(len(self.dataset.classes), 1).T
        assert (heads[0,:] == aux[0]).all(), f"{heads[0,:]}, {aux[0]}"
        heads = heads.reshape(-1)
        assert (aux[0] == heads[:len(self.dataset.classes)]).all(), "heads are not the same"
        
        eval_tails = th.arange(len(self.dataset.classes)).repeat(num_heads).to(self.device)
